...
# Windows in the cmd
    ...
    instructions_fname = "instructions.bat"
    copyCommand = "copy".encode('ascii', 'ignore')
    username = "banool"
    start_local = r"C:\Users\%s\Music\iTunes\iTunes Media" % username
    start_remote = r"\\SERVER\Music\iTunes Media"
    auto_add_location = r"C:Users\%s\Music\iTunes\iTunes Media\Automatically Add to iTunes" % username
    record_location = r"C:\Users\%s\Music\iTunes\record1.txt" % username
    flac_ending = "\\flac_files\\"
    print("Systems except Darwin don't work well currently, and have been disabled.\nExiting.")
    #exit()
else:
...
if(os.path.isfile(record_location)):
    ...
    if system != "Windows":
    	...
    	os.system("""mkdir "%s" """ % record_location)

music_exts = get_extensions(ext_fname, True)


# Stuff for the automount and unmount
...
for i in diff:
    instructions.append("""%s "%s" "%s"\n""" % (copyCommand, i.encode('ascii', 'ignore'), auto_add_location))
...
os.system("%s '%s' ./" % (copyCommand, record_location))
if system != "Windows":
	os.system("rm '%s'" % instructions_fname)
else:
	os.system("del '%s'" % instructions_fname)
